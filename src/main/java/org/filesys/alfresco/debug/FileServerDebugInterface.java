 ...
 * 
 * Adapts the Java file server debug information out to Alfresco's logging
 * 
 * @author gkspencer
 */
public class FileServerDebugInterface implements DebugInterface {
  ...
  private static final Log logger = LogFactory.getLog("org.alfresco.fileServersNG");
  ...
  public void debugPrintln(String str, int level) {
   ...
   if ( level <= getLogLevel())
	...
	public void debugPrintln( Throwable ex, int level) {
		...
		{
				case Debug.Debug:
				  logger.debug( "Debug from JFileServer", ex);
				  break;
				case Debug.Info:
				  logger.info( "Info from JFileServer", ex);
				  break;
				case Debug.Warn:
				  logger.warn( "Warning from JFileServer", ex);
				  break;
				case Debug.Fatal:
				  logger.fatal("Fatal from JFileServer", ex);
				  break;
				case Debug.Error:
     ...
     throws Exception 
  {
    ...
    /**
     * Map logger level to Java file server debug level
     */
