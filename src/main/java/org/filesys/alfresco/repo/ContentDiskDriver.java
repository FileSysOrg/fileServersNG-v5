...
public class ContentDiskDriver extends AlfrescoTxDiskDriver implements DiskInterface, FileLockingInterface, OpLockInterface, DiskSizeInterface
{
    ...
    public static final FileStatus FileUnknown     = FileStatus.Unknown;
    public static final FileStatus FileNotExist    = FileStatus.NotExist;
    public static final FileStatus FileExists      = FileStatus.FileExists;
    public static final FileStatus DirectoryExists = FileStatus.DirectoryExists;
     ...
     * Class constructor
     * 
     * @param smbHelper to connect to the repository services
     */
    public ContentDiskDriver(SMBHelper smbHelper)
    {
    ...
    /**
     * Return the SMB helper
     * 
     * @return SMBHelper
     */
    public final SMBHelper getSMBHelper()
    {
    ...
    public FileInfo getFileInformation(SrvSession session, TreeConnection tree, String path) throws IOException
    {
        ...
        try
        {
            ...
            if ( hasPseudoFileInterface(ctx))
            {
                ...
                if ( fstate == null)
                {
                    ...
                    if ( nodeRef != null)
                    {
                        ...
                        finfo = smbHelper.getFileInformation(nodeRef, isReadOnly, isLockedFilesAsOffline);
                    }
            ...
            if ( finfo == null)
            {
                ...
                finfo = smbHelper.getFileInformation(infoParentNodeRef, infoPath, isReadOnly, isLockedFilesAsOffline);
...
//                		finfo.setAccessDateTime(fstate.getAccessDateTime());
    ...
    public SearchContext startSearch(SrvSession sess, TreeConnection tree, String searchPath, int attributes, EnumSet<SearchFlags> flags) throws FileNotFoundException
    {
        ...
        try
        {
            ...
            List<NodeRef> results = smbHelper.getNodeRefs(searchRootNodeRef, searchFileSpec);
            ...
            if ( searchFileSpec.equals( "*"))
            {
            	...
            	CacheLookupSearchContext cacheContext = new CacheLookupSearchContext(smbHelper, results, searchFileSpec, pseudoList, paths[0], ctx.getStateCache(), isLockedFilesAsOffline);
            	...
            	if ( searchFolderState != null && searchFolderState.hasFilesystemObject())
            	{
            		...
            		FileInfo finfo = smbHelper.getFileInformation((NodeRef) searchFolderState.getFilesystemObject(), isReadOnly, isLockedFilesAsOffline);
            		...
            		if ( searchFolderState.getPath().equals( FileName.DOS_SEPERATOR_STR)) {
            			...
            			finfo = smbHelper.getFileInformation( parentNode, isReadOnly, isLockedFilesAsOffline);
            	...
            	if ( ctx.hasStateCache())
                	searchCtx = new CacheLookupSearchContext(smbHelper, results, searchFileSpec, pseudoList, paths[0], ctx.getStateCache(), isLockedFilesAsOffline);
            	else
                	searchCtx = new ContentSearchContext(smbHelper, results, searchFileSpec, pseudoList, paths[0], isLockedFilesAsOffline);
            }
    ...
    public FileStatus fileExists(SrvSession sess, TreeConnection tree, String name)
    {
        ...
        if (logger.isDebugEnabled() && ctx.hasDebug(AlfrescoContext.DBG_INFO))
            logger.debug("File status determined: name=" + name + " status=" + fstate.getFileStatus().name());
...
//                throw new AccessDeniedException("No delete access to " + params.getFullPath());
            ...
            if ( ctx.hasStateCache())
            {
                ...
                if ( fstate.getOpenCount() > 0 && params.isAttributesOnlyAccess() == false) {
    				...
    				// Check if the required sharing mode is allowed by the current file open
                        ...
                        if ( logger.isDebugEnabled() && ctx.hasDebug(AlfrescoContext.DBG_FILE))
    		    			logger.debug("Local share mode=0x" + Integer.toHexString(fstate.getSharedAccess().intValue()) + ", params share mode=0x" + Integer.toHexString(params.getSharedAccess().intValue()));
    				}
                ...
                if ( nosharing == true)
                {
                    if ( params.getPath().equals( "\\") == false) {
                        ...
                        if ( logger.isDebugEnabled() && ctx.hasDebug(AlfrescoContext.DBG_FILE))
                            logger.debug("Sharing violation path=" + params.getPath() + ", sharing=0x" + Integer.toHexString(fstate.getSharedAccess().intValue()) + ",reason=" + noshrReason);
                ...
                if ( logger.isDebugEnabled() && fstate.getOpenCount() == 0 && ctx.hasDebug(AlfrescoContext.DBG_FILE))
                    logger.debug("Path " + params.getPath() + ", sharing=0x" + Integer.toHexString(params.getSharedAccess().intValue()) + ", PID=" + params.getProcessId());
            }
            ...
            if ( linkRef == null)
            {
                                ...
                                if (( params.isReadWriteAccess() && contentFile.getGrantedAccess() == NetworkFile.Access.READ_WRITE) ||
                                    ...
                                    if ( logger.isDebugEnabled() && ctx.hasDebug(AlfrescoContext.DBG_FILE))
                                        logger.debug("Re-use existing file open Path " + params.getPath() + ", PID=" + params.getProcessId() + ", params=" +
                                                ...
                                                ( contentFile.getGrantedAccess().intValue() <= NetworkFile.Access.READ_ONLY.intValue() ? "ReadOnly" : "Write"));
                                }
                ...
                if ( netFile == null) {
                    ...
                    netFile = ContentNetworkFile.createFile(nodeService, contentService, mimetypeService, smbHelper, nodeRef, params.getPath(), params.isReadOnlyAccess(), params.isAttributesOnlyAccess(), sess);
                }
            }
            else
            {
                ...
                FileInfo fInfo = smbHelper.getFileInformation( nodeRef, isReadOnly, isLockedFilesAsOffline);
...
//                    netFile.setAccessDate( fstate.getAccessDateTime());
        ...
        try
        {
            ...
            Pair<String, NodeRef> result = doInWriteTransaction(sess, new CallableIO<Pair<String, NodeRef>>(){
                public Pair<String, NodeRef> call() throws IOException
                {
                    ...
                    try
                    {
                        nodeRef = smbHelper.createNode(deviceRootNodeRef, path, ContentModel.TYPE_CONTENT);
                        nodeService.addAspect(nodeRef, ContentModel.ASPECT_NO_CONTENT, null);
                    }
                    catch ( FileExistsException ex)
                    {
                        nodeRef = smbHelper.getNodeRef(deviceRootNodeRef, path);
            ...
            ContentNetworkFile netFile = ContentNetworkFile.createFile(nodeService, contentService, mimetypeService, smbHelper, result.getSecond(), params.getPath(), params.isReadOnlyAccess(), params.isAttributesOnlyAccess(), sess);
        ...
        try
        {
            ...
            Pair<String, NodeRef> result = doInWriteTransaction(sess, new CallableIO<Pair<String, NodeRef>>()
            {
                ...
                public Pair<String, NodeRef> call() throws IOException
                {
                    ...
                    NodeRef nodeRef = smbHelper.createNode(deviceRootNodeRef, path, ContentModel.TYPE_FOLDER);
        ...
        try
        {
            NodeRef nodeRef = doInWriteTransaction(sess, new CallableIO<NodeRef>(){
                ...
                public NodeRef call() throws IOException
                {
                    ...
                    NodeRef nodeRef = smbHelper.getNodeRef(deviceRootNodeRef, dir);
                    if (fileFolderService.exists(nodeRef))
                    {
                        // Check if the folder is empty                        
                
                        if ( smbHelper.isFolderEmpty( nodeRef))
                        {                            
    ...
    {  
        ...
        if ( file instanceof ContentNetworkFile) {
            ...
            if ( contentFile.decrementOpenCount() > 0) {
                ...
                logger.debug("Last reference to file, closing, path=" + file.getFullName() + ", access=" + file.getGrantedAccess().name() + ", fid=" + file.getProtocolId() +
            ...
            if (result != null)
            {
                ...
                if (ctx.hasStateCache())
                {
                    if (result.getSecond())
                    {
                        ...
                        fState.setDataStatus(FileState.DataStatus.DeleteOnClose);
                            ...
                            public Void call() throws Exception
                            {
                                ...
                                if (ctx.hasStateCache())
                                {
                                    ...
                                    if (isVersionable == true)
                                    {
                                        ...
                                        delState.setDataStatus(FileState.DataStatus.DeleteOnClose);
                                        delState.setFileStatus(FileStatus.NotExist);
            ...
            throws IOException
    {
        ...
        try
        {
            ...
            boolean isFolder = smbHelper.isDirectory(nodeToMoveRef);
            ...
            if ( isFolder == true || sameFolder == false) {
                ...
                doInWriteTransaction(sess, new CallableIO<Void>()
                {
                    ...
                    {
                        if (sameFolder == true)
                            smbHelper.rename(nodeToMoveRef, name);
                        else
                            smbHelper.move(nodeToMoveRef, sourceFolderRef, targetFolderRef, name);
                        return null;
                    }
                });
            ...
            else 
            {
                ...
                final FileStatus newExists = fileExists(sess, tree, newName);
                ...
                List<Runnable> postTxn = doInWriteTransaction(sess, new CallableIO<List<Runnable>>()
                {
                    ...
                    public List<Runnable> call() throws IOException
                    {
                        ...
                        else if (renameShuffle)
                        {
                            ...
                            if ( newState.getDataStatus() == FileState.DataStatus.Renamed)
                            {
                                ...
                                else
                                {
                                    ...
                                    targetNodeRef = smbHelper.createNode(ctx.getRootNode(), newName, newType);
                            ...
                            else if ( newState.getDataStatus() == FileState.DataStatus.DeleteOnClose)
                            {
                            ...
                            else if ( isFromVersionable == true) 
                            {
                                ...
                                targetNodeRef = smbHelper.createNode(ctx.getRootNode(), newName, nodeService.getType(nodeToMoveRef));
                                    ...
                                    ( targetNodeRef == null || nodeService.hasAspect( targetNodeRef, ContentModel.ASPECT_VERSIONABLE) == false)))
                        {
                            ...
                            postTxn.add(new Runnable()
                            {
                                public void run()
                                {
                                    ...
                                    oldState.setDataStatus(FileState.DataStatus.Renamed);
                                    oldState.setFileStatus(FileStatus.NotExist);
                        ...
                        else 
                        {
                            ...
                            postTxn.add(new Runnable()
                            {
                                ...
                                public void run()
                                {
                                    ...
                                    oldState.setDataStatus( FileState.DataStatus.DeleteOnClose);
                                    oldState.setFileStatus( FileStatus.NotExist);
                                    oldState.setFilesystemObject(nodeToMoveRef);
                    ...
                    if ( info.hasSetFlag(FileInfo.SetDeleteOnClose) && info.hasDeleteOnClose())
                    {
                        ...
                        if (fileFolderService.exists(nodeRef))
                        {
                            ...
                            if ( fstate != null)
                                ...
                                ContentFileInfo cInfo = smbHelper.getFileInformation( nodeRef, isReadOnly, isLockedFilesAsOffline);
                            ...
                            if ( isFolder == true && smbHelper.isFolderEmpty( nodeRef) == false)
