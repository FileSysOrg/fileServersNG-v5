 ...
 * @author gkspencer
 */
public class SMBMounter {

	// Debug logging
	
    private static final Log logger = LogFactory.getLog( SMBMounter.class);
	 ...
	 */
	public SMBMounter(String srvName, String shareName, String userName, String password)
	{
		...
		throws SMBMountException
	{
		...
		case LINUX:
			...
			// Check the protocol type of the SMB server
			
			if ( isProtocolType() == NativeSMB || isProtocolType() == Default)
			{
				// Use the SMB VFS mounter to connect using native SMB
			...
			{
				// Set the command line to use the SMB VFS mounter to connect using NetBIOS
        ...
        if ( logger.isDebugEnabled())
        	logger.debug( "Mount SMB share, cmdLine=" + Arrays.toString(exec.getCommand()));
        ...
        if ( execRes.getSuccess() == false)
        	throw new SMBMountException( execRes.getExitValue(), execRes.getStdOut(), execRes.getStdErr());
	}
